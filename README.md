...
```
—уть которого проста, € преобразовывал svg в dataUri, загружал его через image, рисовал картинку на canvas и превращал в png.  азалось цель достигнута и можно расслабитс€. Ётот подход сработал в Firefox и   Ќо открыв  ~~во всеми нами любом браузере~~ IE, € получил замечательную ошибку:
